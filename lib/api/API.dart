...
class APIInterface {
  ...
  Future<Movie> getTrending(type, time) async {
    ...
    var result = await http.get(
        Uri.parse('${baseUrl}/trending/$type/$time?api_key=${apiKey}&language=ko-KR'));
    var body = jsonDecode(utf8.decode(result.bodyBytes));

    print('${baseUrl}/trending/$type/$time?api_key=${apiKey}&language=ko-KR');
  ...
  Future<Movie> getDiscover() async {
    var result = await http.get(
        Uri.parse('${baseUrl}/discover/movie?api_key=${apiKey}&language=ko-KR'));
    var body = jsonDecode(utf8.decode(result.bodyBytes));

    print('${baseUrl}/discover/movie?api_key=${apiKey}&language=ko-KR');
  ...
  Future<Results> getDetails(String id, String type) async {
    ...
    var result =
    await http.get(Uri.parse('${baseUrl}/$type/$id?api_key=${apiKey}&language=ko-KR'));
    var body = jsonDecode(utf8.decode(result.bodyBytes));
  ...
  Future<Episodes> getEpisode(String id, String type) async {
    ...
    var result =
    await http.get(Uri.parse('${baseUrl}/tv/$id/season/$type?api_key=$apiKey&language=ko-KR'));
    var body = jsonDecode(utf8.decode(result.bodyBytes));
